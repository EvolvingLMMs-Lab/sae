...
if __name__ == "__main__":
    ...
    parser.add_argument("--hf_repo_path", type=str, help="HuggingFace repository path to push to")
    parser.add_argument("--local_path", type=str, default="./data/llava_ov_clevr.parquet")
    parser.add_argument("--target_hf_repo_path", type=str, help="HuggingFace repository path to push to")
    ...
    dataset = load_dataset("lmms-lab/LLaVA-OneVision-Data", name=args.subset, split=args.split)

    original_columns = dataset.column_names
    dataset = dataset.map(convert_llava_ov_dataset, remove_columns=original_columns, num_proc=32)
    
    if args.push_to_hub:
        if not args.target_hf_repo_path:
            raise ValueError("--target_hf_repo_path is required when --push_to_hub is specified")
